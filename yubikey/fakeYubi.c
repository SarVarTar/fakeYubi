...
// Pretend to be Yubikey NFC to unlock more testing features.
#define USB_VENDOR	0x1050
#define USB_PRODUCT	0x0403
...
#define EP_MAX_PACKET_BULK	0x0040
...
// 	.bAlternateSetting =	0,
// 	.bNumEndpoints =	1,
// 	.bInterfaceClass =	3, //USB_CLASS_VENDOR_SPEC,
...
/**/	.bLength =		7, //USB_DT_ENDPOINT_SIZE,
/**/	.bDescriptorType =	5, //USB_DT_ENDPOINT,
/**/	.bEndpointAddress =	0x04, //USB_DIR_OUT | EP_NUM_BULK_OUT,
/**/	.bmAttributes =		3, //USB_ENDPOINT_XFER_BULK,
...
/**/	.bLength =		7, //USB_DT_ENDPOINT_SIZE,
/**/	.bDescriptorType =	5, //USB_DT_ENDPOINT,
/**/	.bEndpointAddress =	0x84, //USB_DIR_IN | EP_NUM_BULK_IN,
/**/	.bmAttributes =		3, //USB_ENDPOINT_XFER_BULK,
/**/	.wMaxPacketSize =	EP_MAX_PACKET_BULK,
/**/	.bInterval =		2,
/**/};
...
/**/	.desc =			{
/**/		{
/**/			.bDescriptorType =	34, //HID_DT_REPORT,
...
int build_config(char *data, int length, bool other_speed) {
  ...
  assert(length >= sizeof(usb_hid));
	...
	memcpy(data, &usb_endpoint_OUT, USB_DT_ENDPOINT_SIZE);
	data += USB_DT_ENDPOINT_SIZE;
	length -= USB_DT_ENDPOINT_SIZE;
	total_length += USB_DT_ENDPOINT_SIZE;

	assert(length >= USB_DT_ENDPOINT_SIZE);
	memcpy(data, &usb_endpoint_IN, USB_DT_ENDPOINT_SIZE);
	data += USB_DT_ENDPOINT_SIZE;
	length -= USB_DT_ENDPOINT_SIZE;
	total_length += USB_DT_ENDPOINT_SIZE;
...
void process_eps_info(int fd) {
	...
	for (int i = 0; i < num; i++) {
		if (assign_ep_address(&info.eps[i], &usb_endpoint_OUT))
			continue;
		if (assign_ep_address(&info.eps[i], &usb_endpoint_IN))
			continue;
	...
	int OUT_addr = usb_endpoint_num(&usb_endpoint_OUT);
	assert(OUT_addr != 0);
	printf("OUT: addr = %u\n", OUT_addr);

	int IN_addr = usb_endpoint_num(&usb_endpoint_IN);
	assert(IN_addr != 0);
	printf("IN: addr = %u\n", IN_addr);
...
/**/			buffer = malloc((__u16)payload_length * sizeof(__u8));
/**/			for(int i = 7; i < 64; i++){
/**/				if(payload_left == 0){
/**/					break;
/**/				}
/**/				buffer[buffer_index] = (__u8)io.data[i];
/**/				buffer_index++;
/**/				payload_left--;
/**/			}
/**/			if((__u16)payload_left > 0){
...
/**/		}
/**/		else{
/**/			printf("getting sequence package: %d\n", io.data[4]);
/**/			for(int i = 5; i < 64; i++){
/**/				buffer[buffer_index] = io.data[i];
/**/				buffer_index++;
/**/				payload_left--;
/**/				if(payload_left == 0){
/**/					buffer_index = 0;
...
bool ep0_request(int fd, struct usb_raw_control_event *event,
	...
	case USB_TYPE_STANDARD:
      ...
      case HID_DT_REPORT:
			...
			break;
		case USB_REQ_SET_CONFIGURATION:
			ep_OUT = usb_raw_ep_enable(fd,
						&usb_endpoint_OUT);
			printf("OUT: ep = #%d\n", ep_OUT);
			ep_IN = usb_raw_ep_enable(fd,
						&usb_endpoint_IN);
			printf("IN: ep = #%d\n", ep_IN);
			...
			// 			&usb_endpoint_int_in);
			// printf("int_in: ep = #%d\n", ep_int_in);
			pthread_create(&ep_OUT_thread, 0,
					ep_OUT_loop, (void *)(long)fd);
			pthread_create(&ep_IN_thread, 0,
					ep_IN_loop, (void *)(long)fd);
